...
#### 线性探测法
- 得到下一个地址 $H(1) = (5 + 1) \mod 11 = 6$，仍然冲突；继续求出 $H(2) = (5 + 2) \mod 11 = 7$，仍然冲突；继续求出 $H(3) = (5 + 3) \mod 11 = 8$，$8$ 对应的地址为空，处理冲突过程结束，记录填入哈希表中序号为 $8$ 的位置。
- code
```Python
def linear_probing(hash_table,key): 
    size = len(hash_table) 
    hash_address  = key % size
    while hash_table[hash_address] is not None : 
        hash_address = (hash_address+1) % size 
...
# 插入38 解决哈希冲突 
linear_probing(hash_table,38)
print("Linear Probing:",hash_table)
```
- ### 二次探测法 
- 得到下一个地址 $H(1) = (5 + 1 \times 1) \mod 11 = 6$，仍然冲突；继续求出 $H(2) = (5 - 1 \times 1) \mod 11 = 4$，$4$ 对应的地址为空，处理冲突过程结束，记录填入哈希表中序号为 $4$ 的位置。
- code
```python
def quadratic_probing(hash_table,key) : 
    ...
    while hash_table[hash_address] is not None : 
        # 正向寻找
        new_hash_address  =   (key + i*i) % size 
        if hash_table[new_hash_address] is None : 
            hash_address = new_hash_address 
            break 
...
keys = [28,49,18]
for key in keys : 
    quadratic_probing(hash_table,key) 
# 插入38 解决哈希冲突 
quadratic_probing(hash_table,38)
print("Quadratic Probing:",hash_table)
```
### 伪随机数序列
- 假设伪随机数为 $9$，则得到下一个地址 $H(1) = (9 + 5) \mod 11 = 3$，$3$ 对应的地址为空，处理冲突过程结束，记录填入哈希表中序号为 $3$ 的位置。
- code

```python
import random 
def pseudo_random_probing(hash_table,key,seed = 42) : 
    random.seed(seed)
    size = len(hash_table) 
    hash_address = key % size 
    while hash_table[hash_address] is not None : 
        
        hash_address = (hash_address + random.randint(1,size-1)) % size 
...
keys = [28,49,18]
for key in keys : 
    pseudo_random_probing(hash_table,key) 
# 插入38 解决哈希冲突 
pseudo_random_probing(hash_table,38)
print("Quadratic Probing:",hash_table)
```
...
class HashTable : 
    ...
    def insert(self,key)  : 
        ...
        if self.table[hash_address] is None : 
              self.table[hash_address] = Node(key) 
        else: 
            current = self.table[hash_address]
...
# 插入键值集合
keys = [88, 60, 65, 69, 90, 39, 7, 6, 14, 44, 52, 70, 21, 45, 19, 32] 
for key in keys: 
...
## 5. leetcdoe热题
1. [哈希表：242.有效的字母异位词](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0242.%E6%9C%89%E6%95%88%E7%9A%84%E5%AD%97%E6%AF%8D%E5%BC%82%E4%BD%8D%E8%AF%8D.md)
2. [哈希表：1002.查找常用字符](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/1002.%E6%9F%A5%E6%89%BE%E5%B8%B8%E7%94%A8%E5%AD%97%E7%AC%A6.md)
3. [哈希表：349.两个数组的交集](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0349.%E4%B8%A4%E4%B8%AA%E6%95%B0%E7%BB%84%E7%9A%84%E4%BA%A4%E9%9B%86.md)
4. [哈希表：202.快乐数](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0202.%E5%BF%AB%E4%B9%90%E6%95%B0.md)
5. [哈希表：1.两数之和](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0001.%E4%B8%A4%E6%95%B0%E4%B9%8B%E5%92%8C.md)
6. [哈希表：454.四数相加II](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0454.%E5%9B%9B%E6%95%B0%E7%9B%B8%E5%8A%A0II.md)
7. [哈希表：383.赎金信](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0383.%E8%B5%8E%E9%87%91%E4%BF%A1.md)
8. [哈希表：15.三数之和](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0015.%E4%B8%89%E6%95%B0%E4%B9%8B%E5%92%8C.md)
9. [哈希表：18.四数之和](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0018.%E5%9B%9B%E6%95%B0%E4%B9%8B%E5%92%8C.md)
10.  [205.同构字符串](https://github.com/youngyangyang04/leetcode-master/blob/master/problems/0205.%E5%90%8C%E6%9E%84%E5%AD%97%E7%AC%A6%E4%B8%B2.md):【哈希表的应用】
## 参考资料
...
- 【书籍】 [代码随想录](https://github.com/youngyangyang04/leetcode-master)
